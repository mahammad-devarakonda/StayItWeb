...
const PostCard = ({ user, onFollowClick, onImageClick, isRequested }) => {
  ...
  return (
    ...
    <div className="flex items-center justify-between p-4">
      ...
      <button
        className={`text-sm font-medium px-4 py-2 rounded-full transition-colors duration-300 border ${getButtonClass()} ${
      ...
      <>
        <div className="px-4 mt-4 flex gap-5">
          <Heart
            role="button"
            aria-label="Like"
            className="cursor-pointer w-6 h-6 text-gray-600 hover:text-red-500 transition"
          />
          <MessageCircle
            role="button"
            aria-label="Comment"
